...
def restaurant():
	...
	fun.tts("Please tell us your customer ID", filename+"custId")
	ph = int(fun.record())
	ph_c = str(ph)
	print("CustomerId: "+ph_c)
	...
	for n in range(0,var.i):
		if var.custid[n]==ph and var.p[n]==0:
			...
			date_string = datetime.now().strftime("%d%m%Y%H%M%S")
			filename = "voice"+date_string
			fun.tts("This is the menu card for hotel Garrison", filename+"menuCard")
			print("-------------------------------------------------------------------------")
			print("						 Hotel AnCasa")
			print("-------------------------------------------------------------------------")
			print("						 Menu Card")
			...
			while(ch!=0):
				...
				date_string = datetime.now().strftime("%d%m%Y%H%M%S")
				filename = "voice"+date_string
				fun.tts("What would you like to have", filename+"orderFile")
			...
			date_string = datetime.now().strftime("%d%m%Y%H%M%S")
			filename = "voice"+date_string
			fun.tts("You have the total bill of "+r, filename+"TotalBill")
			print("Total Bill: ",r)
	...
	if f == 0:
		date_string = datetime.now().strftime("%d%m%Y%H%M%S")
		filename = "voice"+date_string
		fun.tts("Please enter the valid customer id", filename+"InvlidCust")
		print("Invalid Customer Id")
			
		date_string = datetime.now().strftime("%d%m%Y%H%M%S")
		filename = "voice"+date_string
		fun.tts("Say verify again for reverification", filename+"reverification")
		n = str(fun.record()).lower().replace(" ","")
	# n=int(input("0-BACK\n ->"))
	if n=="verifyagain":
		homee.Home()
	else:
